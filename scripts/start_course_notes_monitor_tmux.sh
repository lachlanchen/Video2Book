...
cmd=(bash "$module_root/scripts/monitor_course_notes.sh" --session "$target_session" --interval "$interval" --model "$model" --reasoning "$reasoning" --notes-session-scope "$notes_session_scope")
if [[ -n "$course" ]]; then
  cmd+=(--course "$course")
fi
...
tmux new-session -d -s "$session" -c "$repo_root" "bash -lc 'cd \"$repo_root\" && export NOTES_REPO_ROOT=\"$repo_root\" && NOTE_MONITOR_TMUX_NAME=\"$session\" ${cmd[*]} 2>&1 | tee \"$log_path\"'"
