...
last_session_key=""
while true; do
  next_cmd=(python3 "$module_root/subtitles2notes/generate_course_notes.py" --repo-root "$repo_root" --source-root "$source_root" --print-next)
  if [[ -n "$course" ]]; then
  ...
  gen_cmd=(
    python3 "$module_root/subtitles2notes/generate_course_notes.py"
    --repo-root "$repo_root"
