...
next_pending() {
  local cmd
  cmd=(python3 "$module_root/subtitles2notes/generate_course_notes.py" --repo-root "$repo_root" --source-root "$source_root" --print-next)
  if [[ -n "$course" ]]; then
    cmd+=(--course "$course")
  fi
...
restart_notes() {
  local kill_flag="$1"
  local cmd
  cmd=(bash "$module_root/scripts/start_course_notes_tmux.sh" --session "$session" --session-scope "$notes_session_scope" --no-attach)
  if [[ -n "$course" ]]; then
    cmd+=(--course "$course")
  fi
  if [[ "$allow_partial_course" -eq 1 ]]; then
